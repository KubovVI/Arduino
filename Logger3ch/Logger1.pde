/*
 ...
 ADC0,1,2: Current shunts 12.7ohm - External Currents <87mA
 ADC3: Voltage devider (3M+300K)/300K=11/1 Voltage <12V

 Remark! 
    Errors in SD.writeBlock() will lead to stackoverflow.???
    Don't remove and insert SD-card in Record-mode!!! ???
    Numerous Serial.print lead to stackoverflow.
*/

#define _version "v20160920"
...
#define _dumpDelay 100 //ms
#define _writeAttempts 3
#define _writePause 300 //ms

#define _Vpin 3
#define _cVbat 552 //code Battery
#define _vVbat 6290 //mV Battery
...
void loop() {
  ...
  if (time>=nextTimeLCD) {
    switch (LCDpage){
      ...
      case 3: if (SDerror!=_Ok) LCDerror(SDerror);
              else if (Mode==_Record) LCD_SD(); else LCD_mode(); 
              break;     
      case 4: LCD_Vbat(); break;
...
boolean writeBlock(){
  int attempt=1;
  while (!SD.writeBlock(curBlock,Buffer)){
    attempt++;
    if (attempt>_writeAttempts) return false; 
    delay(_writePause);
  }// while
  return true;
}//writeBlock
...
boolean SDstart(){
  ...
  Serial.print("File"); Serial.println(fileSize);
  Serial.print("Rec="); Serial.println(curRecord);  
  Serial.print("Left="); Serial.println(fileEnd-curBlock+1);  
  return true;
}//SDstart   

boolean closeSD(){
  if (!SDerror==_Ok) return false;
  if (curPos==0) return true;
  for (int i=curPos; i<_blockSize; i++) Buffer[i]=0xFF;
  if (!writeBlock()){ //write data block to SD-card    
    SDerror=_SDfile;
    return false;
  }//if SD.writeBlock
...
// ---------------- LCD_dump ----------------
void LCD_dump(long block){
  lcd.clear();
  lcd.setCursor(0,0); lcd.print("SD dump");
  lcd.setCursor(0,1); lcd.print(block,DEC);  
}//LCD_dump

//------------------------- SD dump by UART ------------------
void dumpSD(){
  // Dump start by "***". Test for final "**". 
  for (int i=0; i<2; i++){
    if (Serial.read()!='*') return; 
  }//for i
  Serial.flush();  
  unsigned int record;
  for (long b=startBlock; b<fileEnd; b++){
    SD.readBlock(b,Buffer);
    record=Buffer[0] | Buffer[1]<<8; // Record # 
    if (record==0xFFFF) break;
    LCD_dump(b-startBlock);
    for (int i=2; i<_blockSize; i++) Serial.write(Buffer[i]);
    delay(_dumpDelay);
  }//for b  
}//dumpSD  
...
// ---------------- LCD_Vbat ----------------
void LCD_Vbat(){
  int v=analogRead(_Vpin); v=map(v,0,_cVbat,0,_vVbat);
  lcd.clear();
  lcd.setCursor(2,0); lcd.print("Vbat");
  lcd.setCursor(0,1); lcd.print(v,DEC); lcd.print("mV"); 
